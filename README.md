...
*   **2026-07-20** — Node pages moved from full prerender to ISR: `/explore` and `/careers` now set `dynamicParams = true` with daily revalidation and prerender only the top 200 nodes by in-degree (deterministic in-degree-desc/id-asc selection shared via `lib/v2/prerender.ts`), long-tail pages render on first request and cache, unknown slugs still 404 — `.next/server/app` drops from 15,085 files / 371 MB to 4,591 files / 123 MB.
