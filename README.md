...
*   **2026-07-21** — The 2026-07-20 V2 scaling audit was verified closed and its docs brought up to date: the hub-page payload fixes (slim parent/child summaries + per-parent context overlays, landed 2026-07-20) hold on the current `main` build — `explore/exam/cat.html` 1,026,820 → 96,826 B, homepage ~704 KB → 27 KB, `.next/server/app` 569 MB/15,067 files → 125 MB/4,613 files, zero dataset strings in client chunks, all 94 vitest tests green. The gitignored review docs (`BUGS.md`, `SCALABILITY_ISSUES.md`) were rewritten from open work orders into a closure report (per-issue commits and after-numbers) so future sessions don't re-chase fixed problems; still open there: the `searchRoutes` maxStates truncation cliff (ISSUE-12), the git data-blob tracking decision (ISSUE-13), and the transient-classed RRB IPv4 citation URLs. `CLAUDE.md`'s known-issues pointer was updated to match.
