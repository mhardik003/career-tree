...
*   **2026-07-04** — Data-architecture v2 migration started; Stage 0 (freeze & foundations) done. The v1 model (full-path string keys, titles as identity — the root cause of the duplicate/divergent-copy problems) is being replaced by a deduplicated role registry + typed transition edges under frozen slug IDs; the full staged design lives in `DATA_ARCHITECTURE_V2.md` one directory above this repo. Shipped: both repos tagged `v1-data-freeze` (masters verified byte-identical); v1 writers guarded (`generate_tree_gemini.py` / `generate_metadata_gemini.py` refuse to run while `migration/FREEZE.md` exists; the private review tool is likewise guarded and `update_tree.sh` is retired — the git-committed JSON here is now the only master); the entity-resolution rubric (`migration/RUBRIC.md`); and a curated 103-exam table (`career-tree/data/exams.json`) that v2 edges will reference by ID so hallucinated exams become validation failures. Also frozen: the entity-resolution evaluation set (`migration/er_labels.json`, 655 labeled title pairs — 355 mechanical rubric applications plus 300 judgment-band pairs labeled by two independent agents each, 94% agreement, 17 flagged for human review) that the Stage-1 resolver is tuned against once and then locked. The suggest/edit APIs stay live — submissions queue in Supabase for the cutover delta pass.
